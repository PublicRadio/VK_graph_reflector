...
export default async function reflexTracksForGroup(remainingTrackCount = 1024) {
    let group = await getNextGroupByTSKey('ts_iteration_key');
    let cursor = 0;

    do {
        let {items: posts} = await callVK('wall.get', {owner_id: -group.id, count, offset: cursor++ * count});
        for (let post of posts) {
            let tracks = post.attachments.map(({audio}) => audio).filter(a => a);
            remainingTrackCount -= tracks.length;
            if (tracks.length > 0) {
                await cypher(`
                MATCH (group:VK_Group {id: ${group.id} })
